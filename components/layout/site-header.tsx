    ...
    <header className="sticky top-3 z-30 px-3 sm:top-4 sm:px-5 lg:px-6">
      <div className="header-shell mx-auto flex min-h-16 w-full max-w-[1180px] items-center gap-3 border border-[rgba(255,209,102,0.24)] bg-[var(--header-bg)] px-3 py-2 backdrop-blur-xl sm:px-4 lg:min-h-[4.5rem]">
        <Link className="focus-ring flex items-center gap-3 rounded-lg" href="/">
          <span className="grid h-10 w-10 shrink-0 place-items-center border border-[rgba(255,209,102,0.42)] bg-[rgba(255,209,102,0.12)]">
            <ShieldCheck aria-hidden className="h-5 w-5 text-[var(--primary)]" />
          </span>
          <span className="whitespace-nowrap text-sm font-black sm:text-lg">TWC Academy</span>
        </Link>
        <nav aria-label="Primary navigation" className="ml-3 hidden items-center gap-1 xl:flex">
          {links.map(([label, href]) => (
            <Link
              className="focus-ring px-3 py-2 text-sm font-semibold text-[var(--muted)] transition hover:bg-[rgba(255,209,102,0.08)] hover:text-[var(--text)]"
              href={href}
              key={href}
            >
              {label}
